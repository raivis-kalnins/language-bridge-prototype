const CACHE='language-bridge-v0.7.0';
const CORE=['./','./index.php','./repair.html','./assets/app-v0.7.0.css','./assets/app-v0.7.0.js','./data/words.json','./manifest.webmanifest','./assets/icons/language-bridge-64.png','./assets/icons/language-bridge-192.png','./assets/icons/language-bridge-512.png'];
...
self.addEventListener('fetch',event=>{
 ...
 if(url.origin!==location.origin||url.pathname.includes('/api/')||url.pathname.endsWith('/admin-api.php'))return;
 const adminRequest=url.search==='?=admin'||url.searchParams.get('admin')==='1'||url.searchParams.get('view')==='admin';
 if(adminRequest||url.searchParams.has('fresh')||url.pathname.endsWith('/version.json')||url.pathname.endsWith('/repair.html')){
